...
1. The *installer* directory already contains a *Product.wxs* files that describes how the product should be deployed on a Windows machine.
...
4. The `candle` command will compile both *.wxs* files to corresponding *.wixobj* binary files.
5. The `light` command will invoke the WiX linker to combine all *.wixobj* files in a single MSI archive.
6. The *x.y.z* sequence represent the target version number. It should typically be something like *3.2.1*.
7. Depending on your processor architecture, you can replace *win-x64* by *win-arm64*.
...
1. As on Windows, the *x.y.z* sequence represent the target version number.
...
It would make sense to distribute the macOS version of this application as a DMG disc image containing a macOS *.app* application bundle. Here is how to proceed.

Lauch a terminal window from the solution's folder and run the following commands:
...
1. As on Windows and Linux, the *x.y.z* sequence represent the target version number.
